...
module Utils.Terminal (selectItems) where
...
selectItems title items = do
  ...
  selected <- loop 0 (replicate (length items) True) items
  hSetBuffering stdin LineBuffering
  hSetEcho stdin True
  return selected

loop :: Int -> [Bool] -> [Text] -> IO [Text]
loop cursor states items = do
  ...
  c <- getChar
  case c of
    '\n' -> cursorDown (length items) >> return [item | (state, item) <- zip states items, state]
