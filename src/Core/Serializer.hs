  ...
  let trimmed = T.strip depStr
   in if T.null trimmed || "--" `T.isPrefixOf` trimmed
        then Nothing
          ...
          let (part1, rest) = T.break (`elem` [' ', ':', '^', '=', '>', '<']) trimmed
          in if T.isPrefixOf ":" rest
             then 
               let alias = part1
                   remainder = T.drop 1 rest
                   (name, constraint) = T.breakOn " " remainder
               in Just $ Dependency
               ...
               let (name, constraint) = T.breakOn " " trimmed
               in Just $ Dependency
                 { depName = unsafeMkPackageName (T.strip name)
                 , depAlias = Nothing
                 , depVersionConstraint = parseVersionConstraint (T.strip constraint)
...
formatDependency dep =
  let name = unPackageName (depName dep)
      base = case depAlias dep of
               Nothing -> name
               Just alias -> alias <> ":" <> name
  in base <> formatVersionConstraint (depVersionConstraint dep)
...
formatVersionConstraint (Just (ExactVersion ver)) = " ==" <> formatVersion ver
formatVersionConstraint (Just (MajorBoundVersion ver)) = " ^>=" <> formatVersion ver
formatVersionConstraint (Just WorkspaceVersion) = ""
formatVersionConstraint (Just (RangeVersion range)) = " " <> formatVersionRange range
formatVersionConstraint (Just (UnparsedVersion v)) = if T.null v then "" else " " <> v
formatVersionConstraint (Just (CabalVersionRange vr)) = " " <> T.pack (CabalPretty.prettyShow vr)
...
formatVersionRange range =
  let lower = case lowerBound range of
      ...
      upper = case upperBound range of
        ...
        Just (ver, Inclusive) -> "<=" <> formatVersion ver
        Just (ver, Exclusive) -> "<" <> formatVersion ver
      connector = if not (T.null lower) && not (T.null upper) then " && " else ""
...
-- Insert a mixin using Surgical Insertion
insertMixinLine :: Text -> Dependency -> Text -> Text -> Text
insertMixinLine eol dep alias content =
  let baseIndent = detectBaseIndent content
      maybeTarget = findMainMixins baseIndent content
  in case maybeTarget of
       Just (indent, before, targetLine, after) ->
         let (mixinsBlock, afterBlock) = spanIndentedBlock indent after
             isEmpty = T.null (T.strip mixinsBlock) && T.null (T.strip (T.dropWhile (== ':') (T.dropWhile (/= ':') targetLine)))
             prefix = if isEmpty then "  " else ", "
             indentStr = T.replicate (indent + 2) " "
             newMixin = indentStr <> prefix <> unPackageName (depName dep) <> " () as " <> alias
             newBlock = targetLine <> eol <> mixinsBlock <> newMixin <> eol
         in before <> newBlock <> afterBlock

       Nothing ->
         let maybeBuildDepends = findMainBuildDepends baseIndent content
         in case maybeBuildDepends of
              Just (indent, before, targetLine, after) ->
                let (depsBlock, afterBlock) = spanIndentedBlock indent after
                    indentStr = T.replicate indent " "
                    newField = indentStr <> "mixins: " <> unPackageName (depName dep) <> " () as " <> alias <> eol
                in before <> targetLine <> eol <> depsBlock <> newField <> afterBlock
              Nothing ->
                let (before, after) = findInsertionPoint baseIndent content
                    indentStr = T.replicate baseIndent " "
                    newField = indentStr <> "mixins: " <> unPackageName (depName dep) <> " () as " <> alias <> eol
                in before <> newField <> after

findMainMixins :: Int -> Text -> Maybe (Int, Text, Text, Text)
findMainMixins baseIndent content = 
  let ls = linesCR content
      matchIndex = findIndexMatches 0 ls
  in case matchIndex of 
       Nothing -> Nothing
  ...
  where
    ...
    findIndexMatches idx (l:ls') =
      ...
      in if "--" `T.isPrefixOf` trimmed
         then findIndexMatches (idx + 1) ls'
         else if "mixins:" `T.isInfixOf` l && currentIndent == baseIndent
              then Just idx
              else findIndexMatches (idx + 1) ls'
