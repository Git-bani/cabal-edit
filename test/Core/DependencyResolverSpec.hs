...
spec = describe "Core.DependencyResolver" $ do
  ...
  describe "resolveVersionConstraint" $ do
    it "preserves user input as UnparsedVersion" $ do
      result <- resolveVersionConstraint Nothing Nothing (trustedMkPackageName "pkg") (Just "^>= 1.2.3") StrategyCaret
    ...
    it "preserves complex constraints without modification" $ do
      result <- resolveVersionConstraint Nothing Nothing (trustedMkPackageName "pkg") (Just ">= 1.2") StrategyCaret
      result `shouldBe` Right (UnparsedVersion ">= 1.2")

    it "rejects invalid version constraints" $ do
      result <- resolveVersionConstraint Nothing Nothing (trustedMkPackageName "pkg") (Just ">>== invalid") StrategyCaret
      case result of
        Left (Error _ InvalidDependency) -> return ()
        _ -> expectationFailure $ "Should have rejected invalid constraint, got: " ++ show result

      result2 <- resolveVersionConstraint Nothing Nothing (trustedMkPackageName "pkg") (Just "foo") StrategyCaret
      case result2 of
        Left (Error _ InvalidDependency) -> return ()
    ...
    it "returns MajorBoundVersion (^>=) when no version is provided and Cabal >= 2.0" $ do
      ...
      result <- resolveVersionConstraint Nothing (Just (Version [2,4])) (trustedMkPackageName "aeson") Nothing StrategyCaret
      case result of
    ...
    it "returns RangeVersion (>= && <) when no version is provided and Cabal < 2.0" $ do
      -- This hits the network/cache
      result <- resolveVersionConstraint Nothing (Just (Version [1,24])) (trustedMkPackageName "aeson") Nothing StrategyCaret
      case result of
