...
spec = describe "Core.DependencyResolver" $ do
  ...
  describe "resolveVersionConstraint" $ do
    it "preserves user input as UnparsedVersion" $ do
      result <- resolveVersionConstraint Nothing (trustedMkPackageName "pkg") (Just "^>= 1.2.3")
    ...
    it "preserves complex constraints without modification" $ do
      result <- resolveVersionConstraint Nothing (trustedMkPackageName "pkg") (Just ">= 1.2")
      result `shouldBe` Right (UnparsedVersion ">= 1.2")

    it "rejects invalid version constraints" $ do
      result <- resolveVersionConstraint Nothing (trustedMkPackageName "pkg") (Just ">>== invalid")
      case result of
        Left (Error _ InvalidDependency) -> return ()
        _ -> expectationFailure $ "Should have rejected invalid constraint, got: " ++ show result

      result2 <- resolveVersionConstraint Nothing (trustedMkPackageName "pkg") (Just "foo")
      case result2 of
        Left (Error _ InvalidDependency) -> return ()
    ...
    it "returns MajorBoundVersion (^>=) when no version is provided" $ do
      ...
      result <- resolveVersionConstraint Nothing (trustedMkPackageName "aeson") Nothing
      case result of
